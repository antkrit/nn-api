...
from api.core.data import Container, Dataset, unpack_x_y
...
def test_dataset():
    ...
    gen = Dataset(x_train, y_train, x_dim=(1, 2), batch_size=4, shuffle=False)
    assert len(gen) == 1

    gen = Dataset(x_train, y_train, x_dim=(1, 2), batch_size=100, shuffle=False)
    assert len(gen) == 1

...
from api.lib.preprocessing.initializers import ones, random_uniform
...
def test_base_optimizer(session, mocker):
    ...
    assert var_ref.value == 1 and var_ref.name == 'added'
    ...
    assert np.array_equal(var.value, 0) and var.name == 'added/added+ref'
    ...
    assert isinstance(var, namespace.nodes.variable)
    assert np.array_equal(var.value, np.ones(var_shape)) and \
           var.name == 'added/added+ref'
    assert var in base.variables()
